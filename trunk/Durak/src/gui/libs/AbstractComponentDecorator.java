...
/**
 * Provide a method for consistently augmenting the appearance of a given component by painting something on it <i>after</i> the component itself gets painted. If not explicitly removed via {@link #dispose}, an instance of this object will
 * live as long as its target component.
 * <p>
 * By default, the decorator matches the location and size of the decorated component, but the bounds can be adjusted by overriding {@link #getDecorationBounds()}. The {@link #synch()} method should be called whenever the bounds returned by
 * {@link #getDecorationBounds()} would change.
 * <p>
 * The decoration is clipped to the bounds set on the decoration, which does not necessarily need to be the same as the decorated component's bounds. The decoration may extend beyond the decorated component bounds, or it may be reduced to a
 * smaller region.
 */
...
public abstract class AbstractComponentDecorator
{
	...
	/**
	 * Account for the difference between the decorator actual origin and the logical origin we want to pass to the {@link #paint} method.
	 */
	...
	/**
	 * Create a decorator for the given component, indicating the layer offset from the target component. Negative values mean the decoration is painted <em>before</em> the target component is painted.
	 */
	...
	/**
	 * Create a decorator with the given position within its layer. Use {@link #TOP} to cover other decorations, or {@link #BOTTOM} to be covered by other decorations.
	 * <p>
	...
	/**
	 * Provide for different tool tips depending on the actual location over the decoration. Note that if you <em>only</em> override this method, you must also invoke {@link #setToolTipText(String)} with a non-<span
	...
	protected void attach()
	{
		...
		RootPaneContainer rpc = (RootPaneContainer) SwingUtilities.getAncestorOfClass(RootPaneContainer.class, component);
	...
	/**
	 * Return any clipping rectangle detected above the given component, in the coordinate space of the given component. The given rectangle is desired to be visible.
	 */
	private Rectangle getClippingRect(Container component, Rectangle desired)
	{
		...
		if (desired.x >= visible.x && desired.y >= visible.y && desired.x + desired.width <= visible.x + visible.width && desired.y + desired.height <= visible.y + visible.height)
		{
		...
		else if (component.getParent() != null)
		{
			...
			if (component != painter.getParent() && (visible.x > 0 || visible.y > 0 || visible.width < component.getWidth() || visible.height < component.getHeight()))
			{
	...
	/**
	 * Return the bounds, relative to the decorated component, of the decoration. The default covers the entire component. Note that this method will be called from the constructor, so be careful when overriding and referencing derived
	 * class state.
	 */
	...
	/**
	 * Change the bounds of the decoration, relative to the decorated component. The special value {@link #DEFAULT_BOUNDS} means the bounds will track the component bounds.
	 */
	...
	/**
	 * Define the decoration's appearance. The point (0,0) represents the upper left corner of the decorated component. The default clip mask will be the extents of the decoration bounds, as indicated by {@link #getDecorationBounds()},
...
//				return getComponent().isShowing();
	...
	/**
	 * Provides a shared background painting mechanism for multiple decorations. This ensures that the background is only painted once if more than one background decorator is applied.
	 */
	...
	private final class Listener extends ComponentAdapter implements HierarchyListener, HierarchyBoundsListener, PropertyChangeListener
	{
