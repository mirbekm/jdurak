...
/**
 * Provides simplified drop handling for a component. Usage:<br>
 * 
 * <pre>
 * &lt;code&gt;
 ...
 * <li>Refuse drops where the user modifiers request an action that is not supported (this works for all cases except when the drag source is not a {@link DragHandler} and the user explicitly requests a MOVE operation; this is
 * indistinguishable from a drag with no modifiers unless we have access to the key modifiers, which {@link DragHandler} provides).
 ...
 * <li>Custom decoration of the drop area may be performed in {@link #paintDropTarget(DropTargetEvent, int, Point)}.
 * </ul>
 * 
 ...
 * <li>{@link #getDropActionsForFlavor(List)} reduces the supported actions based on available flavors. For instance, a text field for file paths might support {@link DnDConstants#ACTION_COPY_OR_MOVE} on a plain string, but
 * {@link DnDConstants#ACTION_LINK} might be the only action supported on a file.
...
public abstract class DropHandler implements DropTargetListener
{
	...
	/**
	 * Create a handler that allows the given set of actions. If using this constructor, you will need to override {@link #isSupported} to indicate which data flavors are allowed.
	 */
	...
	/**
	 * Indicate the actions available for the given list of data flavors. Override this method if the acceptable drop actions depend on the currently available {@link DataFlavor}. The default returns the accepted actions passed into the
	 * constructor.
	 * 
	 * @param dataFlavors
	...
	/**
	 * Calculate the effective action. The default implementation checks whether any {@link DataFlavor}s are supported, and if so, will change the current action from {@link DnDConstants#ACTION_NONE} to something in common between the
	 * source and destination. Refuse user-requested actions if they are not supported (rather than silently accepting a non-user-requested action, which is the Java's DnD default behavior). The drop action is forced to
	 * {@link DnDConstants#ACTION_NONE} if there is no supported data flavor.
	 * <p>
	 * 
	...
	protected int getDropAction(DropTargetEvent e, int currentAction, int sourceActions, int acceptedActions)
	{
		...
		if ((currentAction & acceptedActions) == DragHandler.NONE && !modifiersActive)
		{
			...
			Log.debug("drop action relaxed to " + DragHandler.actionString(action) + " from " + DragHandler.actionString(currentAction));
	...
	/**
	 * Accept or reject the drag represented by the given event. Returns the action determined by {@link #getDropAction(DropTargetEvent)}.
	 */
	...
	/**
	 * Indicates the user has initiated a drop. The default performs all standard drop validity checking and handling, then invokes {@link #drop(DropTargetDropEvent,int)} if the drop looks acceptable.
	 */
	...
	/**
	 * Update the appearance of the target component. Normally the decoration should be painted only if the event is an instance of {@link DropTargetDragEvent} with an action that is not {@link DragHandler#NONE}. Otherwise the decoration
	 * should be removed or hidden.
	 * <p>
	 * For an easy way to highlight the drop target, consider using a single instance of {@link AbstractComponentDecorator} and moving it according to the intended drop location.
	 * 
	 * @param e
	...
	/**
	 * Indicate whether the given drop action is acceptable at the given location. This method is the last check performed by {@link #getDropAction(DropTargetEvent)}. You may override this method to refuse drops on certain areas within the
	 * drop target component. The default always returns true.
	 */
	...
	/**
	 * Handle an incoming drop with the given action. The action passed in might be different from {@link DropTargetDropEvent#getDropAction}, for instance, if there are no modifiers and the default action is not supported. Calling
	 * {@link DropTargetDropEvent#dropComplete} is recommended as soon as the {@link Transferable} data is obtained; this allows the drag source to reset the cursor and any drag images which may be in effect.
	 */
