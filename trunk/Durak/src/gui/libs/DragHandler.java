...
/**
 * Provides simplified drag handling for a component. Usage:<br>
 * 
 * <pre>
 * &lt;code&gt;
...
public abstract class DragHandler implements DragSourceListener, DragSourceMotionListener, DragGestureListener
{
	...
	/**
	 * {@link #getModifiers} returns this value when the current modifiers state is unknown.
	 */
	public static final int UNKNOWN_MODIFIERS = -1;

	/**
	 * {@link #getTransferable(DropTargetDragEvent)} returns this value when the current {@link Transferable} is unknown.
	 */
	...
	/**
	 * Used to communicate the current {@link Transferable} during a drag, if available. Work around absence of access to the data when dragging pre-1.5.
	 */
	...
	/**
	 * Override to provide an appropriate {@link Transferable} representing the data being dragged.
	 */
	...
	public void dragGestureRecognized(DragGestureEvent e)
	{
		if ((e.getDragAction() & supportedActions) != 0 && canDrag(e))
		{
			...
			try
			{
				...
				{
					e.startDrag(cursor, createDragImage(dragIcon), cursorOffset, transferable, this);
				}
				else
				{
					if (dragIcon != null)
					{
						RootPaneContainer rpc = (RootPaneContainer) SwingUtilities.getAncestorOfClass(RootPaneContainer.class, dragSource);
	...
	protected Icon scaleDragIcon(Icon dragIcon, Point offset)
	{
		...
		{
			if (dragIcon.getIconWidth() > maxGhostSize.width || dragIcon.getIconHeight() > maxGhostSize.height)
			{
				int width = dragIcon.getIconWidth();
				dragIcon = new ScaledIcon(dragIcon, maxGhostSize.width, maxGhostSize.height);
